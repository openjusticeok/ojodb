...
ojo_arrests <- function(county = "TULSA",
  ...
  message(paste0(
    "Querying current and archived schemas, sent at ",
    Sys.time()
  ))
  ...
  data_new <- ojo_tbl("arrest", schema = "iic") |>
    filter(
      lubridate::year(booking_date) %in% years,
      !is.na(date)
    ) |>
    select(-updated_at)
  ...
  if (include_charges == TRUE) {
    ...
    data_old_a <- data_old_c |>
      ...
      group_by(.dots = dots) |>
      summarize(
        case_number = first(case_number[which(!is.na(case_number) & case_number != "")]),
